...
class TransformationRecordPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(future: logic.onGetConversionRecord(), builder: (BuildContext context, AsyncSnapshot snapshot){
      if (snapshot.connectionState == ConnectionState.done) {
        return Scaffold(
          ...
          body: Obx(() {
            final List conversionRecord = state.conversionRecord.value;
            return ListView(
              padding: EdgeInsets.all(15),
              children: List.generate(conversionRecord.length, (index) =>  _RecordItem(conversionRecord[index]),),
            );
          }) ,
        );
      }
        ...
        children: [
          Row(
            children: [
              ...
              Text(item.sourceType, style: TextStyle(color: AppColors.COLOR_BLACK_333333, fontSize:AppFontsize.SIZE_46))
            ],
          ),
          Row(
            children: [
              SizedBox(width: 100, child: Text('转账数量', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46)),) ,
              Text(item.changeAmount, style: TextStyle(color: AppColors.COLOR_PRIMARY_D22315, fontSize:AppFontsize.SIZE_46))
            ],
          ),
          Row(
            children: [
              SizedBox(width: 100, child: Text('转账手机号', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46)),),
              Text(item.sourceSn, style: TextStyle(color: AppColors.COLOR_BLACK_333333, fontSize:AppFontsize.SIZE_46))
            ],
          ),
          Row(
            children: [
              SizedBox(width: 100, child: Text('手续费', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46)),),
              Text(item.charges, style: TextStyle(color: AppColors.COLOR_BLACK_333333, fontSize:AppFontsize.SIZE_46))
            ],
          ),
          Row(
            children: [
              SizedBox(width: 100, child: Text('转账时间', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46)),),
              Text(item.createTime, style: TextStyle(color: AppColors.COLOR_BLACK_333333, fontSize:AppFontsize.SIZE_46))
            ],
          ),
        ],
      ),
