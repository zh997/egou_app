...
class WithDrawalRecordPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(future: logic.onGetWithdrawRecord(), builder: (BuildContext context, AsyncSnapshot snapshot){
      if (snapshot.connectionState == ConnectionState.done) {
        return Scaffold(
          ...
          body: Obx(() {
            final List withdrawRecord = state.withdrawRecord.value;
            return ListView(
              padding: EdgeInsets.all(15),
              children: List.generate(withdrawRecord.length, (index) =>  _RecordItem(withdrawRecord[index]),),
            );
          }) ,
        );
      }
         ...
         children: [
           Row(
             children: [
               ...
               Text(item.account, style: TextStyle(color: AppColors.COLOR_BLACK_333333, fontSize:AppFontsize.SIZE_46))
             ],
           ),
           Row(
             children: [
               SizedBox(width: 100, child: Text('提现金额', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46)),) ,
               Text(item.money, style: TextStyle(color: AppColors.COLOR_PRIMARY_D22315, fontSize:AppFontsize.SIZE_46))
             ],
           ),
           Row(
             children: [
               SizedBox(width: 100, child: Text('提现时间', style: TextStyle(color: AppColors.COLOR_GRAY_999999, fontSize:AppFontsize.SIZE_46))),
               Text(item.createTime, style: TextStyle(color: AppColors.COLOR_PRIMARY_D22315, fontSize:AppFontsize.SIZE_46))
             ],
           )
         ],
      ),
